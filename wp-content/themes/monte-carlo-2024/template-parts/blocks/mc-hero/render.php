  ...
  <div class="flex lg:<?php echo esc_html($flex_hash[$alignment]); ?> flex-col md:min-h-[800px] container mx-auto items-center gap-2 pt-36 lg:pt-16 <?php echo esc_html($flex_hash[$alignment]); ?>">
    <div class="flex flex-col lg:w-7/12 text-center lg:text-left">
      ...
      <div class="flex align-middle py-8 space-x-6 justify-center lg:justify-<?php echo $justify_hash[$alignment] ?>">
        <a href="#" data-lookbook-overlay-href="https://resources.montecarlodata.com/data-observability-product-tour" class="mc-btn-primary"><?php echo esc_html($cta1_text) ?></a>
        <a href="/request-a-demo/" class="text-xs sm:text-sm md:text-xl font-serif border-secondary border border-solid px-10 py-2 text-white hover:text-secondary rounded-full"><?php echo esc_html($cta2_text) ?></a>
      </div>
    </div>
    <?php if ($image) : ?>
