...
<div class="bg-gradient-to-b from-[#0073b4] to-[#005180]">
  <div class="flex lg:<?php echo esc_html($flex_hash[$alignment]); ?> flex-col md:min-h-[800px] container mx-auto px-4 items-center gap-2 pt-36 lg:pt-16 <?php echo esc_html($flex_hash[$alignment]); ?>">
    ...
    <a href="/request-a-demo" class="text-xs sm:text-sm md:text-xl font-serif bg-secondary px-8 py-2 rounded-full text-[#1d4b67] hover:text-white"><?php echo esc_html($cta1_text) ?></a>
    ...
    <?php if ($image) : ?>
    <div class="w-5/12 md:-mt-10 lg:-ml-8 pb-10 lg:pb-0"><img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" class="h-auto" /></div>
    <?php endif; ?>
  </div>
</div>
